...
* [一.基础概念](#基础概念)
    - [1.java的垃圾回收器](#1java的垃圾回收器)
    - [2.javadoc](#2javadoc)
    - [3.编码风格](#3编码风格)
    - [4.序列化Serializable](#4序列化Serializable)
...
**垃圾回收器**: java的垃圾回收器，用来监视new所创建的所有对象，并辨别那些不再被引用的对象，随后，释放这些对象的引用空间，以便供其他的对象使用。也就是说，你不用担心内存回收的问题，一旦不再需要，将会自动消失，消除了`内存泄漏`的问题。
